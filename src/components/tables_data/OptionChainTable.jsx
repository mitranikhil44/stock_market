...
export default function OptionChainTable({ snapshots, symbol, underlyingSpot }) {
  ...
  if (!latest?.data?.length) {
    return <div className="text-center text-sm text-gray-400 py-10">No option chain data available.</div>;
  }
    ...
    <div className="w-full overflow-x-auto rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-white/[0.02] backdrop-blur p-1 sm:p-3 shadow-lg">
      <div className="flex flex-wrap items-center justify-between mb-3 gap-2">
        <h3 className="text-lg sm:text-xl font-semibold text-white">
          {symbol.replace(/_/g, " ").split(" ").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")} Option Chain
        </h3>
        ...
        <div className="mb-3 p-2 bg-black/30 rounded-lg text-xs text-gray-300 max-h-[200px] overflow-y-auto">
          ...
          <div className="flex flex-col gap-1">
            {snapshots.map((snap, idx) => (
              <button
                key={idx}
                onClick={() => setManualPrev(snap.timestamp)}
                className={`px-2 py-1 rounded text-left ${
                  manualPrev === snap.timestamp ? "bg-sky-600 text-white" : "hover:bg-sky-800/40"
                }`}
              >
                {snap.timestamp}
              </button>
          ...
          <thead className="bg-black backdrop-blur">            
            <tr>
              <td colSpan={11} className="pb-1 border-white/10">
                <div className="flex items-center justify-center gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    <label className="text-[10px] text-slate-300">Interval</label>
                    <select
                      className="text-[10px] bg-white text-slate-900 border rounded px-1 py-0.5"
                      value={interval}
                      onChange={(e) => { setInterval(e.target.value === "all" ? "all" : Number(e.target.value)); setManualPrev(null); }}
                    >
            ...
            <tr className="text-gray-300 sticky top-0 z-10 bg-black">
              ...
              <th className="py-2 pr-2 text-center border-x">
                Strike {atmStrike != null && <span className="text-[10px] text-emerald-400">(ATM: {atmStrike})</span>}
              </th>
              <th className="py-1 px-1 text-center border-x">Chart OI</th>
          ...
          <tbody>
            {latest.data.map((row, idx) => {
              ...
              return (
                ...
                >
                  <td className="border border-gray-600 px-1 text-right">{cLTP == null ? "-" : cLTP.toLocaleString("en-IN")}</td>
                  <td className="border border-gray-600 px-1 text-right text-gray-300">{cChg}</td>
                  ...
                  <td className="border border-gray-600 px-1 text-center"><MiniChartCell {...{ strike: kStr, metric: "call-oi", snapshots }} /></td>
                  ...
                  <td className="border border-gray-600 px-1 text-center"><MiniChartCell {...{ strike: kStr, metric: "put-oi", snapshots }} /></td>
                  ...
                  <td className="border border-gray-600 px-1 text-left text-gray-300">{putChg}</td>
                  <td className="border border-gray-600 px-1 text-left">{putLTP == null ? "-" : putLTP.toLocaleString("en-IN")}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <p className="mt-2 text-[11px] text-gray-400">Click OI/Vol cells to open a detailed chart & analysis for that strike.</p>
    </div>
  );
}

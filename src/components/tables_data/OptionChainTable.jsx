    ...
    <div className="w-full overflow-x-auto rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-3">
      <div className="max-h-screen overflow-y-auto">
              ...
              return (
                ...
                <tr key={idx}
                  ...
                  <td className="py-1 px-1 text-right align-top">
                    <CellLink symbol={symbol} strike={kStr} metric="call-vol"
                      cur={cVol} prev={pCVol} showPrevInline={showPrevInline} label="Call Vol" />
                  </td>
                  <td className="py-1 px-1 text-right align-top">
                    <CellLink symbol={symbol} strike={kStr} metric="call-oi"
                      cur={cOI} prev={pCOI} showPrevInline={showPrevInline} label="Call OI" />
                  </td>
                  <td className="py-1 px-1 text-center">
                    <MiniChartCell symbol={symbol} strike={kStr} metric="call-oi" snapshots={snapshots}/>
                  </td>
                  <td className="py-1 px-2 font-mono text-gray-200 text-center">
                    {isATM && <span className="inline-block w-2 h-2 rounded-full bg-emerald-400 mr-1"/>}
                    {kStr}
                  </td>
                  <td className="py-1 px-1 text-center">
                    <MiniChartCell symbol={symbol} strike={kStr} metric="put-oi" snapshots={snapshots}/>
                  </td>
                  <td className="py-1 px-1 text-left align-top">
                    <CellLink symbol={symbol} strike={kStr} metric="put-oi"
                      cur={pOI} prev={ppOI} showPrevInline={showPrevInline} label="Put OI" />
                  </td>
                  <td className="py-1 px-1 text-left align-top">
                    <CellLink symbol={symbol} strike={kStr} metric="put-vol"
                      cur={putVol} prev={pPutVol} showPrevInline={showPrevInline} label="Put Vol" />
                  </td>
        ...
        <p className="mt-2 text-[11px] text-gray-400">
          Click OI/Vol cells to open a detailed graph & analysis for that strike.
        </p>
      </div>
    </div>
  );
